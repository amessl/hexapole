...
BoTorch-based pipeline for 13-parameter hexapole ion transfer / 3-objective (EIC intensity of three mass signals) process.
...
5. **Measure & ingest** — after running the experiment, fill in the empty
   ...
   appended to the main CSV with the next index numbers and the surrogate model can be immediately refit and suggest the next batch of candidates.

## CLI usage

```bash
# 1. Get the first batch of 5 candidates from the initial samples (generated by LHS and subsequent measurements)
